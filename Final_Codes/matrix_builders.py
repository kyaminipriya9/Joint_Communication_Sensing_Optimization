...
# Build the quantization-side U matrix for AP l and user k.Used in ADC quantization-related kappa terms.
def compute_Uq_lk(A_l,G_lk,W_lk):
    return G_lk.conj().T @ W_lk.conj().T @ A_l
# Build the quantization-side V matrix for AP l and user k.
...
# Build D^Th_lk used in the pilot-power-dependent thermal noise term
...
#Build D^ADC_lk used in the pilot-power-dependent ADC distortion term.
...
#Compute B @ inv(C) without explicitly forming inv(C).This is numerically better than using np.linalg.inv(C).
def right_solve(B, C):
...
######## Below are the optional codes (currently not used anywhere in implementation) that are kept for reference. ########
