...
# =============================================================================
# Run-mode dispatch and lazy deep-unfolding imports
# =============================================================================
# MODE HELPER: returns True for all pure communication modes so sensing tensors can be skipped.
...
# IMPORT DISPATCH: loads the correct DU implementation only when the selected mode needs it.
def load_unfolding_api(mode):
    ...
    elif mode in ["joint", "joint_comm_sensing"]:
...
# =============================================================================
# Communication kappa and hardware-noise coefficient construction
# =============================================================================
# INTERNAL HELPER: extracts all AP/panel entries for one fixed user index k.
...
# COEFFICIENT BUILDER: assembles all communication kappa coefficients used by SINR/WSR updates.
...
# COEFFICIENT BUILDER: computes pilot-power-independent thermal and ADC constants.
...
# HARDWARE MODEL: builds AP-wise ADC quantization covariance matrices for a reference power split.
...
# =============================================================================
# Communication SINR, WSR, and FP/QT auxiliary updates
# =============================================================================
# SINR HELPER: desired-signal numerator coefficient for user k.
def compute_Ak(k, kappa_S, P_tilda):
    return P_tilda[k] * kappa_S[k]


# SINR HELPER: pilot-power-dependent denominator coefficient for user k.
...
# SINR HELPER: pilot-power-independent denominator coefficient for user k.
...
# METRIC: evaluates exact communication SINR for one user.
...
# DIAGNOSTIC PRINT: shows desired, interference, distortion, and noise components per user.
...
# FEASIBILITY HELPER: clips pilot power when data power is fixed.
...
# FEASIBILITY HELPER: clips data power when pilot power is fixed.
...
# METRIC: computes weighted communication spectral efficiency from per-user SINR.
...
# FP/QT UPDATE: updates the communication auxiliary variable mu_k.
...
# FP/QT BLOCK UPDATE: closed-form pilot-power update for fixed data power.
...
# FP/QT BLOCK UPDATE: closed-form data-power update for fixed pilot power.
...
# =============================================================================
# Power-budget projection and clipping helpers
# =============================================================================
# INTERNAL HELPER: converts scalar/vector Pmax into a length-K vector.
...
# PROJECTION HELPER: Euclidean projection onto a nonnegative simplex budget.
...
# FEASIBILITY PROJECTION: projects P_bar, P_tilda, and P_prime jointly under each user budget.
...
# OPTIONAL SAFETY FALLBACK: can reset infeasible users to their EPA initialization values.
...
# FEASIBILITY HELPER: clips pilot power while holding data and sensing powers fixed.
...
# FEASIBILITY HELPER: clips data power while holding pilot and sensing powers fixed.
...
# FEASIBILITY HELPER: clips sensing power while holding communication powers fixed.
...
# =============================================================================
# Sensing coefficient loading and MATLAB-matching coefficient builders
# =============================================================================
# DATA SHAPE HELPER: converts MATLAB KxTg/TgxK sensing matrices into Tg x K.
...
# OPTIONAL DATA LOADER: loads precomputed sensing coefficients when the dataset contains them.
...
# INTERNAL HELPER: forces vectors into complex column-vector shape.
def _col(x):
    return np.asarray(x, dtype=complex).reshape(-1, 1)


# INTERNAL HELPER: fetches the steering vector for AP l and target q.
def _get_b_lq(b_target_all, l, q):
    return _col(b_target_all[:, l, q])


# INTERNAL HELPER: builds the per-AP sensing combiner list for one target.
...
# SENSING COEFFICIENT: desired target-return coefficient a_{q,k}.
...
# SENSING COEFFICIENT: inter-target interference coefficient for b_{q,k}.
...
# SENSING COEFFICIENT: clutter coefficient for one target and MC iteration.
...
# SENSING NOISE FLOOR: DAC distortion contribution for one target and MC iteration.
...
# SENSING NOISE FLOOR: thermal-noise contribution for one target.
...
# SENSING NOISE FLOOR: ADC quantization contribution for one target.
...
# SENSING BUILDER: averages MATLAB-matching sensing coefficients over combiner/MC iterations.
...
# =============================================================================
# Sensing objective, LDT/QT auxiliary variables, and sensing power update
# =============================================================================
# SENSING METRIC: computes target-wise desired signal S_q and denominator D_q.
...
# SENSING METRIC: computes target-wise sensing SINR.
def compute_sensing_sinr(P_prime, a_sens, b_sens, n_sens):
    S, D = compute_sensing_S_D(P_prime, a_sens, b_sens, n_sens)
    return S / (D + 1e-30)


# SENSING METRIC: computes weighted sensing objective contribution.
...
# LDT UPDATE: sets sensing auxiliary mu_q equal to sensing SINR_q.
...
# QT UPDATE: updates target-wise sensing quadratic-transform auxiliary y_q.
...
# INTERNAL HELPER: derivative used in the exact coordinate update for P_prime.
...
# SENSING BLOCK UPDATE: solves each sensing-power coordinate by bisection.
def update_P_prime(
    P_prime,
    P_bar,
    P_tilda,
...
# SENSING BLOCK UPDATE: repeats coordinate updates until the P_prime block stabilizes.
...
# =============================================================================
# Joint objective evaluation and monotone step acceptance
# =============================================================================
# JOINT METRIC: combines weighted communication WSR and sensing objective.
...
# STATE EVALUATION: computes all objective components for a candidate power allocation.
...
# MONOTONICITY HELPER: backtracks the three-block update until the joint objective is accepted.
...
# REPORT DATA: expands sensing output into desired, interference, clutter, noise, SINR, and rates.
...
# =============================================================================
# Classical joint communication-sensing FP/QT optimizer
# =============================================================================
# MAIN CLASSICAL OPTIMIZER: runs the joint FP/QT block-coordinate loop.
...
# =============================================================================
# MATLAB data loading and array conversion helpers
# =============================================================================
# DATA LOADING HELPER: finds the first available variable name from a list of MATLAB aliases.
...
# DATA LOADING HELPER: selects one setup when arrays include a setup dimension.
...
# UNIT HELPER: converts dB power to linear scale.
def db2lin(db):
    return 10.0 ** (db / 10.0)


# UNIT HELPER: converts linear values to dB with numerical protection.
def lin2db(x):
    return 10.0 * np.log10(np.maximum(x, 1e-30))


# MATRIX HELPER: symmetrizes a complex matrix to reduce numerical asymmetry.
def hermitianize(X):
    return 0.5 * (X + X.conj().T)


# DIAGNOSTIC HELPER: computes relative Frobenius error between matrices.
def relerr(A, B):
...
# DATA CONVERSION: converts MATLAB N x N x K x M arrays into [M][K] lists.
...
# DATA CONVERSION: converts MATLAB N x N x M arrays into per-AP lists.
...
# DATA CONVERSION: converts mean channels into [M][K] column-vector lists.
...
# DATA CONVERSION: converts pilot/code matrices into a per-user list.
...
# COMBINER BUILDER: creates identity/MR communication combiners.
...
# =============================================================================
# Optional sensing-projection diagnostics and MR/V-combiner utilities
# =============================================================================
# INTERNAL HELPER: forces vectors into complex column-vector shape.
def _col(x):
    return np.asarray(x, dtype=complex).reshape(-1, 1)
...
# SENSING DATA HELPER: returns target steering vector b_{l,q}.
...
# OPTIONAL DIAGNOSTIC HELPER: builds MR sensing combiners v_{l,q}=b_{l,q}.
...
# SENSING DATA HELPER: extracts MATLAB clutter-aware sensing combiners for one MC iteration.
...
# OPTIONAL DIAGNOSTIC HELPER: estimates target variances from reference covariance matrices.
...
# OPTIONAL DIAGNOSTIC: computes scalar desired sensing power for MR-style checks.
...
# OPTIONAL DIAGNOSTIC: computes scalar inter-target sensing interference.
...
# OPTIONAL DIAGNOSTIC: computes scalar clutter sensing term.
...
# OPTIONAL DIAGNOSTIC: computes scalar DAC sensing distortion.
...
# OPTIONAL DIAGNOSTIC: computes scalar thermal sensing noise.
...
# OPTIONAL DIAGNOSTIC: computes scalar ADC sensing distortion.
...
# OPTIONAL DIAGNOSTIC: computes all scalar sensing components using MR combiners.
...
# OPTIONAL DIAGNOSTIC: computes sensing components using MATLAB V-combiner iterations.
...
# DIAGNOSTIC PRINT: displays target-wise sensing projection components.
...
# LEGACY DATA LOADER: loads one setup and converts MATLAB arrays for the legacy runner.
...
# =============================================================================
# Console reporting and formatted table helpers
# =============================================================================
# REPORT HELPER: formats scalar values consistently for console tables.
...
# REPORT HELPER: prints horizontal table separators.
def print_rule(width=118, char="-"):
    print(char * width)


# REPORT HELPER: prints standardized section headings.
...
# REPORT HELPER: prints compact key/value tables.
...
# REPORT HELPER: displays the initial per-user power split.
...
# REPORT HELPER: displays per-user optimized power allocations.
...
# REPORT HELPER: displays target-wise sensing coefficient/rate information.
...
# REPORT HELPER: compares sensing performance before and after optimization.
...
# REPORT HELPER: displays communication and sensing objective weights.
...
# REPORT HELPER: sums a power vector robustly.
def _sum_power(x):
    return float(np.sum(np.asarray(x, dtype=float)))


# REPORT HELPER: prepares communication/sensing/joint objective table rows.
...
# REPORT HELPER: prints objective contributions by component.
...
# REPORT HELPER: prints the power/setup context for a result dictionary.
...
# REPORT HELPER: prints communication component breakdown from stored kappas.
...
# REPORT HELPER: prints target-wise sensing component breakdown.
...
# REPORT HELPER: prints the full before/after optimization report.
...
# =============================================================================
# Standalone classical communication-only and sensing-only baselines
# =============================================================================
# BASELINE OPTIMIZER: optimizes P_bar and P_tilda while sensing power is fixed.
...
# BASELINE OPTIMIZER: optimizes P_prime while communication powers are fixed.
...
# REPORT HELPER: prints a compact joint-result summary.
...
# UTILITY: pads variable-length convergence histories for averaging/plotting.
...
# DATA HELPER: loads target weights from the dataset or defaults to all ones.
...
# =============================================================================
# Legacy one-setup runner supporting joint, communication-only, and DU modes
# =============================================================================
# LEGACY RUNNER: executes one setup/power case with classical, sensing-only, and optional DU paths.
...
# =============================================================================
# Deep-unfolding training utilities and learned-step-size reporting
# =============================================================================
# TRAINING HELPER: creates shuffled mini-batches of setup indices.
...
# TRAINING LOOP: legacy joint-DU training loop over powers, epochs, and setup batches.
...
# MODEL INSPECTION: extracts learned step sizes from the DU model.
...
# MODEL INSPECTION: prints layer-wise learned DU step sizes.
...
# =============================================================================
# Plotting utilities for convergence and power-sweep summaries
# =============================================================================
# PLOTTING: saves a convergence comparison between classical FP/QT and DU.
...
# PLOTTING: saves power-sweep comparison of EPA/classical/DU results.
...
# REPORT HELPER: formats vectors for compact text/debug reports.
...
# =============================================================================
# Legacy batch-training path for joint deep unfolding
# =============================================================================
# LEGACY TRAINING STEP: trains one batch through the legacy run_one_setup_power path.
def train_one_batch(
    mat_file,
    model,
    optimizer,
...
# =============================================================================
# Legacy power-sweep runner
# =============================================================================
# LEGACY SWEEP: runs the older power-sweep pipeline across powers and setups.
...
# =============================================================================
# Classical-only sweep wrapper and clean-mode data pipeline
# =============================================================================
# CLASSICAL SWEEP: runs classical-only tests without DU training.
...
# CLEAN DATA LOADER: loads and converts one setup for the clean-mode pipeline.
...
# =============================================================================
# Clean-mode communication problem-context builder
# =============================================================================
# CLEAN CONTEXT BUILDER: prepares all fixed communication matrices/kappas for one setup.
...
# =============================================================================
# Clean-mode communication-only classical/lambda-QT runners
# =============================================================================
# CLEAN COMM RUNNER: runs classical communication-only FP/QT using precomputed kappas.
...
# CLEAN COMM-LAMBDA RUNNER: compares FP/QT, D/lambda-QT, and optional DU for one setup.
...
# CLEAN COMM RUNNER: runs communication-only FP/QT and optional DU for one setup.
...
# =============================================================================
# Clean-mode joint sensing-coefficient builder and joint runner
# =============================================================================
# CLEAN SENSING BUILDER: builds sensing coefficients for a clean joint context.
...
# CLEAN JOINT RUNNER: runs joint communication-sensing FP/QT and optional DU for one setup.
...
# =============================================================================
# Deep-unfolding model creation, checkpointing, and step-size loading
# =============================================================================
# DU FACTORY: creates the correct communication-only or joint unfolding model for the run mode.
...
# ARTIFACT OUTPUT: saves learned DU step sizes as text and NPZ files.
...
# ARTIFACT OUTPUT: saves the full trained DU model checkpoint.
...
# MODEL LOADER: restores learned step sizes into a fresh DU model.
...
# =============================================================================
# Clean-mode DU forward pass, batch training, and evaluation helpers
# =============================================================================
# CLEAN DU HELPER: runs one DU forward pass from a prepared clean context.
...
# CLEAN TRAINING HELPER: builds tensors and initial powers for one setup/power.
...
# CLEAN TRAINING STEP: trains one mini-batch for the selected clean run mode.
...
# CLEAN TRAINING LOOP: trains DU step sizes according to communication or joint mode.
...
# CLEAN EVAL HELPER: appends DU evaluation to a classical/lambda result.
...
# =============================================================================
# Clean-mode single-setup test runner and final power sweep
# =============================================================================
# CLEAN SINGLE-SETUP RUNNER: dispatches one setup/power to communication, lambda, or joint mode.
...
# CLEAN POWER SWEEP: averages results across test setups and saves convergence plots.
...
# =============================================================================
# Script entry point: configure training, save learned steps, and run testing
# =============================================================================
# ENTRY POINT: runs the configured train/save/test workflow when this file is executed directly.
if __name__ == "__main__":
    ...
    num_pgd_steps = 5
    init_step_bar = 0.02
    init_step_tilda = 0.02
    init_step_prime = 0.02
    learning_rate = 1e-3
    num_epochs = 10
    batch_size = 10
