...
def build_sensing_coefficients_matlab_matching(
    ...
    return a_sens, b_sens, n_sens
# def build_sensing_coefficients_from_geometry(
#     T,
#     alpha_list,
#     sigma_d2,
#     A_list,
...
# ):
#     """
#     Geometry-based fallback for a_{qk}, b_{qk}, n_q.

#     This matches the coefficient structure in the derivation:
#         S_q(P') = sum_k a_{qk} P'_k
#         D_q(P') = sum_k b_{qk} P'_k + n_q

#     If V_combiner_all is supplied, coefficients are averaged over its fourth
#     dimension. Otherwise MR sensing v_{l,q}=b_{l,q} is used.
#     """
#     alpha_list = np.asarray(alpha_list, dtype=float).reshape(-1)
#     sigma_d2 = np.asarray(sigma_d2, dtype=float).reshape(-1)
#     alpha_var_matrix_all = np.asarray(alpha_var_matrix_all, dtype=float)
...
#     a_sens = np.zeros((Tg, K), dtype=float)
#     b_sens = np.zeros((Tg, K), dtype=float)
#     n_sens = np.zeros(Tg, dtype=float)

#     if V_combiner_all is None:
#         v_iters = [None]
#     else:
#         v_iters = list(range(V_combiner_all.shape[3]))

#     for q in range(Tg):
#         for v_iter in v_iters:
#             v_list = _build_v_list_for_q(
#                 b_target_all=b_target_all,
#                 q=q,
#                 V_combiner_all=V_combiner_all,
#                 v_iter=v_iter,
...
#                     b_lq = _get_b_lq(b_target_all, l, q)
#                     desired_gain = np.abs((v_lq.conj().T @ A_l @ b_lq)[0, 0]) ** 2
#                     desired_k += alpha_var_matrix_all[k, l, q] * desired_gain

#                     for t in range(Tg):
#                         if t == q:
#                             continue
#                         b_lt = _get_b_lq(b_target_all, l, t)
#                         inter_gain = np.abs((v_lq.conj().T @ A_l @ b_lt)[0, 0]) ** 2
#                         inter_k += alpha_var_matrix_all[k, l, t] * inter_gain

#                     C_dir_lk = C_h_dir_all[l][k]
#                     cval = v_lq.conj().T @ A_l @ C_dir_lk @ A_l.conj().T @ v_lq
#                     clutter_k += np.real(cval[0, 0])

#                 scale = (T ** 2) * (alpha_list[k] ** 2)
#                 a_sens[q, k] += scale * desired_k
#                 b_sens[q, k] += scale * (inter_k + clutter_k)
               

#             # Fixed floor n_q = P_DAC,q^ref + P_Th,q^ref + P_ADC,q^ref
#             pdac_q = 0.0
#             pth_q = 0.0
#             padc_q = 0.0
#             for l in range(M):
...
#                 pth_q += np.real((v_lq.conj().T @ A_l @ A_l.conj().T @ v_lq)[0, 0])
#                 padc_q += np.real((v_lq.conj().T @ Theta_list[l] @ v_lq)[0, 0])

#                 for k in range(K):
#                     C_tot_lk = C_h_total_all[l][k]
#                     pdac_val = v_lq.conj().T @ A_l @ C_tot_lk @ A_l.conj().T @ v_lq
#                     pdac_q += sigma_d2[k] * np.real(pdac_val[0, 0])

#             n_sens[q] += T * (pdac_q + sigma_w2 * pth_q + padc_q)

#         # Average over V-combiner MC iterations if applicable.
#         a_sens[q, :] /= len(v_iters)
#         b_sens[q, :] /= len(v_iters)
#         n_sens[q] /= len(v_iters)

#     return a_sens, b_sens, n_sens


# ============================================================
# Sensing SINR, WSR, LDT, QT, and P'_k update
# ============================================================
...
def update_P_prime(
    ...
    for k in range(K):
        if Pmax is None:
            # Without a budget, use the current value as a conservative upper seed.
            # For your joint problem P_total_max should normally be provided.
            upper = max(P_new[k], 1.0)
...
# def compute_sensing_components_from_coeffs(P_prime, a_sens, b_sens, n_sens, target_weights=None, w_s=1.0):
#     P_prime = np.asarray(P_prime, dtype=float).reshape(-1)
...
#     S, D = compute_sensing_S_D(P_prime, a_sens, b_sens, n_sens)
#     interference = np.maximum(b_sens @ P_prime, 0.0)
...
def run_joint_wsr_optimisation(
    ...
    for t in range(max_iters):
        # ---------- Communication LDT/QT and communication powers ----------
        ...
        # # ---------- Raw sensing update, no joint-budget clipping yet ----------
        # mu_s = update_sensing_mu(P_prime_old, a_sens, b_sens, n_sens)
        ...
        #     target_weights=target_weights,
        #     w_s=w_s,
        #     P_total_max=None,          # important: no budget clipping here
        #     eps_power=eps_power,
        # )

        # # ---------- Fair joint projection ----------
        # P_bar, P_tilda, P_prime = project_joint_three_power_blocks(
        #     P_bar_raw=P_bar_raw,
        #     P_tilda_raw=P_tilda_raw,
        #     P_prime_raw=P_prime_raw,
        #     P_total_max=P_total_max,
        #     eps=eps_power,
        # )
        # ---------- Final exact objective evaluation ----------
        P_th = P_bar * kappa_Th1 + kappa_Th0
        P_adc = P_bar * kappa_ADC1 + kappa_ADC0
        ...
        if verbose:
            print(
                ...
                f"comm={comm_wsr:.6f} | sensing={sensing_wsr:.6f} | delta={delta:.3e}"
            )
...
def run_wsr_optimisation(
    ...
    return wsr_history, P_bar, P_tilda, gamma
...
def print_sensing_power_components(label, sensing, w_s=1.0):
    ...
    for q in range(len(S)):
        print(
            ...
            f"{_fmt_float(D[q])} | "
            f"{_fmt_float(sinr[q])} | "
            # f"{_fmt_float(sinr_db[q])} | "
            f"{_fmt_float(rate[q])} | "
            # f"{_fmt_float(weighted[q])}"
        )
    ...
    print(
        ...
        # f"{_fmt_float(np.sum(weighted))}"
    )
# def print_sensing_power_components(label, sensing, w_s=1.0):
#     """Print sensing power components from the coefficient model."""
#     if sensing is None:
#         return
#     S = np.asarray(sensing.get("S", []), dtype=float).reshape(-1)
#     if len(S) == 0:
#         return
#     I = np.asarray(sensing.get("I", np.full_like(S, np.nan)), dtype=float).reshape(-1)
#     N_floor = np.asarray(sensing.get("N", np.full_like(S, np.nan)), dtype=float).reshape(-1)
#     D = np.asarray(sensing.get("D", I + N_floor), dtype=float).reshape(-1)
#     sinr = np.asarray(sensing.get("SINR", S / (D + 1e-30)), dtype=float).reshape(-1)
#     sinr_db = np.asarray(sensing.get("SINR_dB", lin2db(sinr)), dtype=float).reshape(-1)
#     rate = np.asarray(sensing.get("rate", np.log2(1.0 + np.maximum(sinr, 0.0))), dtype=float).reshape(-1)
#     omega = np.asarray(sensing.get("omega", np.ones_like(rate)), dtype=float).reshape(-1)
#     weighted = float(w_s) * omega * rate

#     print_heading(label, width=118)
#     print(
#         f"{'q':>3} | {'S desired':>13} | {'I/clutter':>13} | {'noise floor':>13} | "
#         f"{'D total':>13} | {'SINR':>13} | {'SINR(dB)':>13} | {'rate':>13} | {'weighted':>13}"
#     )
#     print_rule(132)
#     for q in range(len(S)):
#         print(
#             f"{q:3d} | {_fmt_float(S[q])} | {_fmt_float(I[q])} | {_fmt_float(N_floor[q])} | "
#             f"{_fmt_float(D[q])} | {_fmt_float(sinr[q])} | {_fmt_float(sinr_db[q])} | "
#             f"{_fmt_float(rate[q])} | {_fmt_float(weighted[q])}"
#         )
#     print_rule(132)
#     print(
#         f"{'SUM':>3} | {_fmt_float(np.sum(S))} | {_fmt_float(np.sum(I))} | {_fmt_float(np.sum(N_floor))} | "
#         f"{_fmt_float(np.sum(D))} | {'':>13} | {'':>13} | {_fmt_float(np.sum(rate))} | "
#         f"{_fmt_float(np.sum(weighted))}"
#     )
...
def run_one_setup_power(
    ...
    check_sensing=False,
    print_sensing=True,
    init_delta=0.50,      # sensing fraction
    init_lambda=0.50,     # pilot fraction inside communication power
    ...
    P_prime_init = delta0 * P_total_max * np.ones(K)

    P_comm_init = (1.0 - delta0) * P_total_max
    ...
    # a_sens, b_sens, n_sens = build_sensing_coefficients_from_geometry(
    #     T=tau,
    #     alpha_list=alpha_list,
    #     sigma_d2=sigma_d2,
    #     A_list=A_list,
    ...
    # sensing_coeff_source = "python_geometry"
    ...
    print("sum inter_coeff per target =", np.sum(sensing_coeff_parts["inter_coeff"], axis=1))
    print("clutter_coeff per target =", sensing_coeff_parts["clutter_coeff"])
...
def run_joint_power_sweep(
    ...
    if save_plots:
        ...
        plt.ylabel("Mean joint objective")
        plt.title("Mean Joint Objective vs Power over Test Setups")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.savefig("joint_mean_objective_vs_power.png", dpi=300)
        plt.show()
...
if __name__ == "__main__":
    ...
    # For multiple setups, use: test_setups = np.arange(0, 10)
    test_setups = np.arange(0, 1)
