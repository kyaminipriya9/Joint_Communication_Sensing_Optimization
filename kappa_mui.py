...
def compute_kappa_M1_PP(j, k,
                        ...
                        T):
    """
    Pilot-power MUI coefficient for victim user k and interferer j.

    It returns M1[k,j] such that

        P_MUI,k contains:
            P_tilda[j] * P_bar[k] * M1[k,j]

    This is the same coefficient used by the DP form.
    """
    ...
    ZjHZk = Z_list[j].conj().T @ Z_list[k]
    delta_jk = np.linalg.norm(ZjHZk, "fro")**2 / d
    ...
    for l in range(L):
        ...
        Sigma_lk = Sigma_list[l][k]
        Sigma_lj = Sigma_list[l][j]

        Cref_lk = C_h_ref_all[l][k]
        Cref_lj = C_h_ref_all[l][j]

        inner += np.real(
            np.trace(Sigma_lk @ U_lk @ Sigma_lj @ V_lk)
            + np.trace(Sigma_lk @ U_lk @ Cref_lj @ V_lk)
            + np.trace(Cref_lk @ U_lk @ Sigma_lj @ V_lk)
            + np.trace(Cref_lk @ U_lk @ Cref_lj @ V_lk)
        )

    return delta_jk * (alpha_list[j]**2) * (alpha_list[k]**2) * (T**2) * inner

def compute_kappa_M0_PP(alpha_j, tau, k, j,
                     U_list, V_list,
                     Sigma_list,       # 2-D [l][user]
                     R_list,           # 2-D [l][user]
                     h_bar_list,       # 2-D [l][user]
                     C_h_ref_all,      # 2-D [l][user]
                     C_h_total_all,    # 2-D [l][user]  C_{h,li} = Sigma_li + C_ref_li
                     sigma_d2,         # 1-D [user]
                     sigma_w2,
                     U_q_list, V_q_list, Theta_list):
    """
    kappa_M0[k, j] — highlighted terms from eq. (7)

    Inputs:
        alpha_j        : scalar alpha for interferer j
        tau            : scalar T
        k              : desired user index
        j              : interferer user index
        U_list         : 2-D [l][user]
        V_list         : 2-D [l][user]
        Sigma_list     : 2-D [l][user]  Sigma_dir
        R_list         : 2-D [l][user]
        h_bar_list     : 2-D [l][user]  shape (N,1)
        C_h_ref_all    : 2-D [l][user]
        C_h_total_all  : 2-D [l][user]  C_{h,li} = Sigma_lk + C_ref_lk
        sigma_d2       : 1-D [user]
        sigma_w2       : scalar
    """
    L = len(U_list)
    ...
    # ── Highlight 1: i≠j interference sum ────────────────────────
    for l in range(L):
        U_lk     = U_list[l][k]
        V_lk     = V_list[l][k]
        Sigma_lj = Sigma_list[l][j]
        ...
        for i in range(K):
            ...
            Sigma_li = Sigma_list[l][i]
            Cref_li  = C_h_ref_all[l][i]

            t1 = np.trace(Sigma_li @ U_lk @ Sigma_lj @ V_lk)
            t2 = np.trace(Sigma_li @ U_lk @ Cref_lj  @ V_lk)
            t3 = np.trace(Cref_li  @ U_lk @ Sigma_lj @ V_lk)
            t4 = np.trace(Cref_li  @ U_lk @ Cref_lj  @ V_lk)

            kappa += alpha_j**2 * sigma_d2[i] * tau * (t1 + t2 + t3 + t4)

    # ── Highlight 2: i=j self term ────────────────────────────────
    for l in range(L):
        U_lk     = U_list[l][k]
        V_lk     = V_list[l][k]
        Sigma_lj = Sigma_list[l][j]
        ...
        t1 = np.trace(Sigma_lj @ U_lk @ Sigma_lj @ V_lk)
        t2 = np.trace(R_lj @ U_lk) * np.trace(R_lj @ V_lk)
        t3 = np.trace(R_lj @ U_lk) * np.real(h_lj.conj().T @ V_lk @ h_lj)
        t4 = np.real(h_lj.conj().T @ U_lk @ h_lj) * np.trace(R_lj @ V_lk)

        kappa += alpha_j**2 * sigma_d2[j] * tau * (t1 + t2 + t3 + t4)

    # ── Noise term ────────────────────────────────────────────────
    for l in range(L):
        U_lk        = U_list[l][k]
        V_lk        = V_list[l][k]
        C_h_lj      = C_h_total_all[l][j]   # C_{h,lj}

        kappa += alpha_j**2 * sigma_w2 * tau * np.trace(V_lk @ U_lk @ C_h_lj)

    # ── Quantization term (optional) ─────────────────────────────
    for l in range(L):
        C_h_lj = C_h_total_all[l][j]
        kappa += (
            alpha_j**2 * tau *
            np.trace(U_q_list[l][k] @ C_h_lj @ V_q_list[l][k] @ Theta_list[l])
        )
    ...
    for l in range(L):
        for m in range(L):
            if l == m:
                continue
            C_h_lj = C_h_total_all[l][j]
            C_h_mj = C_h_total_all[m][j]

            kappa += (
                alpha_j**2 * sigma_d2[j] * tau *
                np.trace(U_list[l][k] @ C_h_lj) *
                np.trace(V_list[m][k] @ C_h_mj)
            )
